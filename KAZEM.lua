          ...
          send(msg.chat_id_, 0, 1, "● - <code>الايدي 📍 : </code><i>"..msg.sender_user_id_.."</i>\n● - <code>الروابط تم قفلها ممنوع ارسالها</code> ⚠️❌", 1, 'html')
            ...
            if text then
              ...
              if database:get('bot:spam:warn'..msg.chat_id_) and string.len(text) > (sens) or ctrl_chars > (sens) or real_digits > (sens) then
                delete_msg(chat,msgs)
          send(msg.chat_id_, 0, 1, "● - <code>الاي����������ي ���� : </code><i>"..msg.sender_user_id_.."</i>\n● - <code>الكلايش تم قفلها ممنوع ارسالها</code> ⚠️❌", 1, 'html')
              end
          end 
...
sendContact(msg.chat_id_, msg.id_, 0, 1, nil, 18507570359, "آلمہؤسہيہقہآر✨🎼آلحہزيہنہ🌞🌸" , "", bot_id)
end
...
if text == 'هلو' then 
if not database:get('bot:rep:mute'..msg.chat_id_) then
moody = "• بَلِياكِ أَنِي مختنك بِدَمُوعَيْ كُلّ ساعَة ساعَة احترك مَلَّيتِ مَن هَجَّرَكَ تَرْه كَلُبِّي بِغُيّابكِ مايدكعَلِيّ الساعَة اباوع وانتضر حَبِيبَيْيَ شُو مالِك خَبَّرَ كَلُبِّي ٱِشْتَعَلَ مَثَل الجَمْر الفركة خنكتني خُنّكَِٰ😾🌈ֆ"
else 
moody = ''
end
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
...
if text == 'غني' then 
if not database:get('bot:rep:mute'..msg.chat_id_) then
moody = "•ماكو مثله وبحلاه وفدوه كل عمري فداه بهدي روحي وقلبي له وبمشي انا على هواه مثله ماكو بالجمال وكِنت اشوفه بالخيال وكِنت قول انا محال مثله عالدنيا يجينيֆ"
else
moody = ''
end
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
...
if text == 'هم غني' then 
if not database:get('bot:rep:mute'..msg.chat_id_) then
moody = "• صدق جالي لو حلم وربي لا اقسم قسم قلبي اله مابينقسم وابقى بمره كل سنيني💚ֆ"
else 
moody = ''
end
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
...
if text == 'غنيلي' then 
if not database:get('bot:rep:mute'..msg.chat_id_) then
moody = "•ربي انطاني ملاك سبب لقلبي هلاك يعني ماتعرف حبيبي عندي وشكبره غلاك غلاك انت بقلبي كله وحبك انسى مامله وعني مايغيب عني امشي وياه مثل ضله😍"
else 
moody = ''
end
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
...
send(msg.chat_id_, msg.id_, 1, '● - `العضو` *'..result.sender_user_id_..'* `بالفعل ت�� الغاء كتمه` ☑️', 1, 'md')
end
	else
         database:srem(hash, result.sender_user_id_)
                  if database:get('lang:gp:'..msg.chat_id_) then
